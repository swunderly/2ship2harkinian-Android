...
static std::shared_ptr<Ship::ControllerButton> GetControllerButtonForPort(uint8_t port, N64ButtonMask bitmask) {
    ...
    return controller->GetButton(bitmask);
}
...
void BenInputEditorWindow::DrawButtonLineAddMappingButton(uint8_t port, N64ButtonMask bitmask) {
    auto button = GetControllerButtonForPort(port, bitmask);
    if (button == nullptr) {
        ImGui::BeginDisabled();
        ImGui::Button(StringHelper::Sprintf("%s###addButtonMappingButton%d-%d", ICON_FA_PLUS, port, bitmask).c_str(),
                      ImVec2(SCALE_IMGUI_SIZE(20.0f), 0.0f));
        ImGui::EndDisabled();
        if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNormal | ImGuiHoveredFlags_NoSharedDelay)) {
            ImGui::SetTooltip("Button is unavailable for this controller");
        }
        return;
    }
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX && button->AddOrEditButtonMappingFromRawPress(bitmask, "")) {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
...
void BenInputEditorWindow::DrawButtonLineEditMappingButton(uint8_t port, N64ButtonMask bitmask, std::string id) {
    ...
    auto mapping = button->GetButtonMappingById(id);
    if (mapping == nullptr) {
        return;
    }
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX && button->AddOrEditButtonMappingFromRawPress(bitmask, id)) {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
