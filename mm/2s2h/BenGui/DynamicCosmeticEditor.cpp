...
static std::shared_ptr<Ship::Archive> GetArchiveFromFile(Ship::ArchiveManager* archiveManager,
                                                         const std::string& filePath) {
    if (archiveManager == nullptr) {
        return nullptr;
    }

    for (const auto& archive : archiveManager->GetArchives()) {
        if (archive != nullptr && archive->HasFile(filePath)) {
            return archive;
        }
    }
...
static bool IsSkeletonOverriddenByCustomArchive(Ship::ArchiveManager* archiveManager, const char* path) {
    ...
    return IsCustomArchive(GetArchiveFromFile(archiveManager, path));
}
...
static bool TryLoadCustomDisplayListXml(Ship::ArchiveManager* archiveManager, Ship::ResourceManager* resourceManager,
                                        ...
                                        std::shared_ptr<LUS::DisplayList>& material, tinyxml2::XMLElement*& root) {
    ...
    material = std::dynamic_pointer_cast<LUS::DisplayList>(resourceManager->LoadResource(materialPath));
    return material != nullptr;
}

static size_t FindDisplayListInstructionIndex(const LUS::DisplayList& displayList, const Gfx& expected,
                                              size_t searchStart) {
...
void ApplyDynamicCosmetics() {
    ...
    for (const auto& entry : customCosmeticEntries) {
        ...
        for (const auto& binding : entry.bindings) {
            if (!IsCustomArchive(GetArchiveFromFile(archiveManager.get(), binding.materialPath))) {
                continue;
            }

            auto material =
                std::dynamic_pointer_cast<LUS::DisplayList>(resourceManager->LoadResource(binding.materialPath));
...
void ScanDynamicCosmetics() {
    ...
    for (const auto& materialPath : *materialPaths) {
        if (!IsCustomArchive(GetArchiveFromFile(archiveManager.get(), materialPath))) {
            continue;
        }

        tinyxml2::XMLDocument document;
        std::shared_ptr<LUS::DisplayList> material;
