...
void Player_AnimationPlayOnceReverse(PlayState* play, Player* player, PlayerAnimationHeader* anim);
...
static void FinishOcarinaPlay(PlayState* play) {
    ...
    player->actor.flags &= ~ACTOR_FLAG_20000000;
    if (player->unk_A90 != NULL) {
        player->unk_A90->flags &= ~ACTOR_FLAG_20000000;
    }
    player->stateFlags2 &= ~PLAYER_STATE2_8000000;
    ...
    if (player->actionFunc == Player_Action_63) {
        ...
        Player_AnimationPlayOnceReverse(play, player, D_8085D17C[player->transformation]);
    }
}
...
static bool NeedsOcarinaCleanup(EnGs* enGs, PlayState* play) {
    ...
    bool stoneInvolved = (enGs->unk_19A & 0x200) || (player->unk_A90 == &enGs->actor);

    if (!stoneInvolved) {
        return false;
    }

    return (player->actor.flags & ACTOR_FLAG_20000000) || (player->stateFlags2 & PLAYER_STATE2_8000000) ||
           (play->msgCtx.ocarinaMode != OCARINA_MODE_NONE) ||
