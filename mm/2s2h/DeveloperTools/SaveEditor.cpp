...
void DrawTempleClears() {
    ...
    if (UIWidgets::Checkbox("Woodfall Open", &open, UIWidgets::CheckboxOptions().Disabled(cleared))) {
        if (open) {
            SET_WEEKEVENTREG(WEEKEVENTREG_20_01);
        } else {
    ...
    if (UIWidgets::Checkbox("Snowhead Open", &open, UIWidgets::CheckboxOptions().Disabled(cleared))) {
        if (open) {
            SET_WEEKEVENTREG(WEEKEVENTREG_30_01);
        } else {
    ...
    if (UIWidgets::Checkbox("Great Bay Open", &open, UIWidgets::CheckboxOptions().Disabled(cleared))) {
        if (open) {
            SET_WEEKEVENTREG(WEEKEVENTREG_53_20);
            SET_WEEKEVENTREG(WEEKEVENTREG_93_08);
        } else {
    ...
    if (UIWidgets::Checkbox("Stone Tower Inverted", &inverted,
                            UIWidgets::CheckboxOptions()
                                .Disabled(!inStoneTower)
                                .DisabledTooltip("Can only invert while in Stone Tower"))) {
        if (inverted) {
...
void DrawPlayerTab() {
    ...
    if (gPlayState) {
        ...
        if (ImGui::BeginTable("stateTable", 4)) {
            ...
            ImGui::PushStyleColor(ImGuiCol_TableHeaderBg, UIWidgets::ColorValues.at(formColor));
